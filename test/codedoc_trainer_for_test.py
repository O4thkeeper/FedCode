...
from test import bleu_for_test
...
class CodeDocTrainer:
    ...
    def test(self):
        ...
        (goldMap, predictionMap) = bleu_for_test.computeMaps(predictions,
                                                             os.path.join(self.args.output_dir, "test.gold"))
        data, all_score, score, num = bleu_for_test.bleuFromMaps(goldMap, predictionMap)

        assert len(data) == len(all_score) == len(score) == num
        # dev_bleu = round(bleu.bleuFromMaps(goldMap, predictionMap)[0], 2)
        # logging.info("  %s = %s " % ("bleu-4", str(dev_bleu)))

        with open(os.path.join(self.args.output_dir, 'codedoc_result_for_paper.txt'), 'a') as f:
            for i in range(num):
                f.write("%s\n%s\n%s\n%s\n\n" % (score[i], all_score[i], data[i][0], data[i][1]))
