...
def process_data_and_test(test_raw_examples, test_model, preprocessor, args, test_batch_size=1000):
    ...
    for batch_idx, batch_data in enumerate(batched_data):
        if len(batch_data) < test_batch_size or batch_idx > 0:
            break
        examples = []
        ...
        assert len(examples) == test_batch_size
        ...
        with open(os.path.join(args.output_dir, 'mrr_result_for_paper.txt'), 'a') as f:
            f.write("TEST TIME:%s\n\n" % time.asctime(time.localtime(time.time())))
            f.write("rank:\n%s\n\n" % ranks)
            f.write("logits:\n%s\n\n" % all_logits)
            f.write("scores:\n%s\n\n" % scores.tolist())
        with open(os.path.join(args.output_dir, 'mrr_data_for_paper.txt'), 'a') as f:
            f.write("TEST TIME:%s\n\n" % time.asctime(time.localtime(time.time())))
            for e in examples:
                f.write("%s\n%s\n\n" % (e.text_a, e.text_b))
