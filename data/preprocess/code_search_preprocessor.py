...
from data.preprocess.utils.code_search_utils import convert_examples_to_features, temp_convert_examples_to_features


class CodeSearchPreprocessor(BasePreprocessor):
    def __init__(self, args, label_vocab, tokenizer):
        super(CodeSearchPreprocessor, self).__init__()
        self.args = args
    ...
    def temp_transform(self,examples):
        label_list = self.get_labels()
        output_mode = "classification"
        features = temp_convert_examples_to_features(examples, label_list, self.args.max_seq_length, self.tokenizer,
                                                output_mode, cls_token_at_end=bool(self.args.model_type in ['xlnet']),
                                                # xlnet has a cls token at the end
                                                cls_token=self.tokenizer.cls_token, sep_token=self.tokenizer.sep_token,
                                                cls_token_segment_id=2 if self.args.model_type in ['xlnet'] else 1,
